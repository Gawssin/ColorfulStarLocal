...
int main(int argc, char **argv)
{
    ...
    cout << "Version: " << (basicFlag ? "Basic (HStarDP)" : "Advanced (HStarCD)") << endl
         << endl;
    ...
    // for (int i = 0; i < g.n; i++)
    // {
    //     printf("%d -> %s\n", i, _int128_to_str(ColofulStarCoreNum[i]));
    // }
    ...
    __int128 maxCoreNum = 0;
    for (int i = 0; i < g.n; i++)
    {
        if (ColofulStarCoreNum[i] > maxCoreNum)
            maxCoreNum = ColofulStarCoreNum[i];
    }
    
    cout << _int128_to_str(maxCoreNum) << endl;
