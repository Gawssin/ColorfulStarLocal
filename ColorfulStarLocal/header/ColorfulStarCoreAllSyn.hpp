...
void ColorfulStarHIndex(Graph &g, __int128 **dp, int h, int *color, int **CC, __int128 *ColofulStarCoreNum, int colorNum, int optimization)
{
    ...
    for (int i = 0; i < g.n; i++)
    {
        cout << _int128_to_str(uBoundLast[i]) << " ";
    }
    printf("\n");
    while (updateFlag)
    {
        ...
        for (int i = 0; i < g.n; i++)
        {
            ...
            if (optimization >= 3)
            {
                ...
                if (lastPos[i] != -1)
                {
                    ...
                    {
                        int nbrID = g.adj[j];
                        if (uBoundLast[nbrID] < uBoundLast[i])
                        {
                            updatedi = true;
                            break;
                        }
            ...
            //__int128 lastBound = uBound[i];
            compHIndex(g, i, dp, uBound, h, color, CC, colorNum, idUBThreads[threadId], NotColorThreads[threadId], MustColorThreads[threadId],
                       uBoundLast, lastPos);
            // if(uBound[i] > uBoundLast[i])
            // {
            //     cout << " ++++++++++++++++++++++++++++ " << i << _int128_to_str(uBoundLast[i]) << " " << _int128_to_str(uBound[i]) << endl;
            // }
