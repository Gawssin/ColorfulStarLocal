...
void ColorfulStarCoreDecomp(Graph &g, __int128 **dp, int h, int *color, int **CC, __int128 *ColofulStarCoreNum, int colorNum, int basicFlag, __int128 *maxCore = 0, int *maxCoreNum = 0)
{
    ...
    __int128 curMin = maxStarDegree;

    int *delNodes = new int[g.n];
    int delNum = 0;
    bool *mark = new bool[g.n]();

    printf("Times\t\tLeft_Nodes\tMaxCore(N)\tMaxCore(M)\tMaxCore(Density)\tMaxCore(CoreNumber)\n");
    
    int *conveBound = new int[g.n];
    
    int *numPerIteration = new int[g.n](); 
    ...
    while (leftN > 0)
    {
        ...
        curMin = maxStarDegree;
        for (int i = 0; i < g.n; i++)
        {
            if(mark[i] == false)
                curMin = min(curMin, tmpDP[i]);
        }
        
        for (int i = 0; i < g.n; i++)
        {
            if (mark[i] == false && tmpDP[i] == curMin)
                {
                    delNodes[delNum++] = i;
                    mark[i] = true;
                    conveBound[i] = times - 1;
                }
        }
        numPerIteration[times - 1] = delNum;

        if (curMin > starCoreNum)
        {
            starCoreNum = curMin;
        }

        for (int q = 0; q < delNum; q++)
        {
            leftN--;
            int i = delNodes[q];
            ...
            for (int j = g.cd[i]; j < g.cd[i] + g.deg[i]; j++)
            {
                ...
                //curMin = min(curMin, dp[nbr][h - 1]);
                tmpDP[nbr] = dp[nbr][h - 1];
    ...
    // FILE *input = fopen("conve.out", "r");
    // int id, round;
    // for (int i = 0; i < g.n; i++)
    // {
    //     fscanf(input, "%d%d", &id, &round);
    //     assert(conveBound[id] >= round);
    // }
    // fclose(input);
    
    
    // FILE *output = fopen("tmptheo.out", "w");
    // int totalNum = 0;
    // for (int i = 0; i < times; i++)
    // {
    //     totalNum += numPerIteration[i];
    //     fprintf(output, "%lf\n", 1.0 * totalNum / g.n);
    // }
    ...
    printf("End:\n%d\t\t%d\t\t%s\n", times, leftN , _int128_to_str(starCoreNum));
