...
#pragma omp parallel for schedule(dynamic, numThreads) reduction(+: invokeNum, unchangeNum, remainNum)
        for (int i = 0; i < g.n; i++)
        {
            ...
            uBound[i] = compHIndex(g, i, dp, uBound, h, nbrUB, color, CC, colorNum,
                idUBThreads[threadId], NotColorThreads[threadId], MustColorThreads[threadId], lastPos, lastId, boundCore, &prune);
